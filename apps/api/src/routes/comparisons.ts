...
comparisonsRouter.post("/comparisons", async (request, response, next) => {
  try {
...
comparisonsRouter.get("/comparisons", async (request, response, next) => {
  try {
...
comparisonsRouter.get("/comparisons/:id", async (request, response, next) => {
  try {
...
async function executeVariant(comparisonId: string, variant: "A" | "B", model: ModelId, prompt: string, input: string) {
  const outcome = await runModel(model, prompt, input);
